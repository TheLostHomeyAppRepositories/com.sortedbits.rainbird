...
export class RainBirdClient {
  ...
  private async sendRequest(request: RainBirdRequest): Promise<Response | undefined> {
    if (this.showRequestResponse) {
      this.log.log(`[${this.address}] Request:  ${request.type}`);
    }
    ...
    while (true) {
      try {
        ...
        this.log.log(`RainBird controller request failed. [${error}]`);
        this.log.log(`Failed Request: ${request.type}`);
        if (!request.retry) {
          break;
        }
        this.log.log(`Will retry in ${this.RETRY_DELAY} seconds`);
  ...
  private getResponse(encryptedResponse: Buffer): Response | undefined {
    ...
    if (this.showRequestResponse) {
      this.log.log(`[${this.address}] Response: ${response ?? 'Unknown'}`);
    }
