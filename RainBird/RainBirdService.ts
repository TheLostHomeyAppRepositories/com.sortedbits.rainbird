...
export class RainBirdService extends events.EventEmitter {
  ...
  get currentZoneId(): number {
    return this._currentZoneId;
  }

  get currentZoneRemainingDuration(): number | undefined {
    if (this._currentZoneId && this._zones[this._currentZoneId]) {
      const zone = this._zones[this._currentZoneId];
      return zone.remainingDuration;
    }
    return undefined;

  }

  get currentZoneDurationTime(): string | undefined {
    const currentDuration = this.currentZoneRemainingDuration;
    if (currentDuration) {
      return this.formatTime(currentDuration);
    } else {
      return undefined;
    }
  }

  get client(): RainBirdClient {
    return this._client;
  }
  ...
  async init(): Promise<RainBirdMetaData> {
    ...
    if (!irrigationState) {
      this.log.log('RainBird controller is currently OFF. Please turn ON so plugin can control it');
    }
  ...
  activateZone(zone: number, duration: number): void {
    this.log.log(`Zone ${zone}: Activate for ${duration} seconds`);
  ...
  async deactivateZone(zone: number): Promise<void> {
    this.log.log(`Zone ${zone}: Deactivate`);
  ...
  async startProgram(programId: string): Promise<void> {
    this.log.log(`Program ${programId}: Start`);
  ...
  private async startZone(zone: number, duration: number): Promise<void> {
    this.log.log(`Zone ${zone}: Start for ${duration} seconds`);
    ...
    try {
      ...
      if (!this.isActive(zone)) {
        this.log.log(`Zone ${zone}: Skipped as it is not active`);
        return;
      }
      ...
      if (this.isInUse(zone)) {
        this.log.log(`Zone ${zone}: Skipped as it is already in use`);
        return;
      }

      this.log.log(`Zone ${zone}: Start [Duration: ${this.formatTime(duration)}]`);
      ...
      this.log.log(`Zone ${zone}: Failed to start [${error}]`);
    } finally {
      this._statusRefreshSubject.next();
    }
  }
  ...
  private setStatusTimer(): void {
    ...
    if (timerDuration > 0) {
      this.log.log(`Status timer set for ${timerDuration} secs`);
  ...
  private async performStatusRefresh(): Promise<void> {
    try {
      ...
      this.log.log(`Failed to get status: ${error}`);
    }
  }
  ...
  private async setControllerDateTime(): Promise<void> {
    ...
    this.log.log(`Adjusting Rainbird Controller Date/Time from ${controller.toLocaleString()} to ${host.toLocaleString()}`);
  ...
  public async setIrrigationDelay(days: number): Promise<void> {
    this.log.log(`Set Irrigation Delay: ${days} days`);
  ...
  private async updateStatus(): Promise<void> {
    ...
    if (previousZoneId !== 0 && this._zones[previousZoneId].running && previousZoneId !== currentZone?.id) {
      this.log.log(`Zone ${previousZoneId}: Complete`);
    }
    ...
    if (previousProgramId !== undefined && previousProgramId !== '' && previousProgramId !== this._currentProgramId) {
      this.log.log(`Program ${previousProgramId}: Complete`);
    }

    if (this._currentProgramId !== undefined && this._currentProgramId !== '' && previousProgramId !== this._currentProgramId) {
      this.log.log(`Program ${this._currentProgramId}: Running [Time Remaining: ${this.formatTime(status.program?.timeRemaining)}]`);
    }

    if (currentZone !== undefined && currentZone.running && previousZoneId !== currentZone.id) {
      this.log.log(`Zone ${currentZone.id}: Running [Time Remaining: ${this.formatTime(currentZone.timeRemaining)}]`);
    }
    ...
    if (this._rainSetPointReached !== status.rainSensorSetPointReached) {
      ...
      this.log.log(`Rain Sensor: ${status.rainSensorSetPointReached ? 'SetPoint reached': 'Clear'}`);
    }
  }
  ...
  private async displaySupportWarning(page0: Buffer): Promise<void> {
    ...
    this.log.log('This plugin does not fully support your RainBird model and may not not correctly show the zone\'s state such as time remaining');
    this.log.log('If you would like better support please create a GitHub issue [https://github.com/donavanbecker/homebridge-rainbird/issues]');
    this.log.log('and supply the following details:');
    this.log.log(`  Model: ${this.model}, Zones: ${[...this.zones.keys()]}`);
    this.log.log(`  ProgramZoneState Page 0: ${[...page0.values()]}`);
    this.log.log(`  ProgramZoneState Page 1: ${[...page1.values()]}`);
    this.log.log(`  ProgramZoneState Page 2: ${[...page2.values()]}`);
    this.log.log('Also include your model (if different to the one above), which program is running and');
    this.log.log('the time remaining for the currently running zone as well as for the other idle/waiting zones');
  }
